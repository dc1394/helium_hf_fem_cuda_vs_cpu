module Helium_HF_FEM_Eigen
    ...
    function make_wavefunction(iter, hfem_param, hfem_val, vh_val)
        ...
        norm_cpu = norm((hfem_val.hg * phi - hfem_val.ug * eigenval[1] * phi)[:,1])
        norm_gpu = norm((hfem_val.hg * h_VA - hfem_val.ug * h_W[1] * h_VA)[:,1])

        @printf "最低固有値（CPU） = %.16f\n" eigenval[1]
        @printf "最低固有値（GPU） = %.16f\n" h_W[1]
        @printf "ノルム（CPU）= %.16e\n" norm_cpu
        @printf "ノルム（GPU）= %.16e\n" norm_gpu
        @printf "最低固有値 CPUとGPUの差 = %.16e\n" abs(eigenval[1] - h_W[1])
        ...
        return eigenval[1], norm_cpu, norm_gpu
    end
